...
if (imgs.length) {
  ...
  useEffect(() => {
    ...
    progRef.current = setInterval(() => {
      setProgress(p => {
        ...
        return p + (100 / (AUTO_MS / 60))
      })
    }, 60)
    autoRef.current = setInterval(() => goTo(prev => prev + 1), AUTO_MS)
    return () => { clearInterval(autoRef.current); clearInterval(progRef.current) }
  }, [current, images.length, paused, goTo])

  /* thumbnail strip scroll-to-active */
  useEffect(() => {
  ...
  if (loading) return (
    <section style={{
      ...
      <div style={{
        width: 40, height: 40, border: '3px solid var(--blue-pale)',
    ...
    <section style={{ background: 'var(--off-white)', padding: '0 0 3rem' }}>
      ...
      <div style={{ position: 'relative', maxWidth: 860, margin: '0 auto', padding: '0 1rem' }}>
        ...
        <div style={{
          position: 'absolute', top: '50%', left: '50%',
          transform: 'translate(-50%,-50%)',
          width: 'min(580px, 82vw)', height: 'min(580px, 82vw)',
          borderRadius: '50%',
          border: '1px dashed rgba(41,128,185,0.18)',
          pointerEvents: 'none', zIndex: 0,
        }} />

        {/* Main image frame — circle */}
        <div
        ...
        {/* Prev / Next buttons — positioned outside circle */}
        <button
