...
export default function StorePage() {
  ...
  async function checkout() {
    if (!user) { navigate('/signin'); return }
    try {
      await storeApi.createOrder({ shippingAddress: null })
      setCart([])
      setCartOpen(false)
      navigate('/portal')
    } catch (err) {
  ...
  const cartTotal   = cart.reduce((s, i) => {
    const price = i.product_variants?.price ?? i.products?.sale_price ?? i.products?.price ?? 0
    ...
    <div className="page-wrapper">
      ...
      <div className="page-hero" style={{ background: 'var(--green-deep)' }}>
        ...
        <p className="section-desc" style={{ color: 'rgba(255,255,255,0.75)', maxWidth: 500 }}>
          Books, workbooks, digital tools, and more — curated for your healing journey.
        </p>
      </div>
      ...
      <div style={{ background: 'var(--white)', padding: '1.25rem 2rem', borderBottom: '1px solid var(--earth-cream)', display: 'flex', gap: '1rem', alignItems: 'center', flexWrap: 'wrap', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 10 }}>
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', flex: 1 }}>
          <button onClick={() => { setActiveCategory('all'); setPage(1) }}
            style={{ padding: '0.4rem 1rem', borderRadius: '100px', border: `1.5px solid ${activeCategory === 'all' ? 'var(--green-deep)' : 'var(--earth-cream)'}`, background: activeCategory === 'all' ? 'var(--green-deep)' : 'var(--white)', color: activeCategory === 'all' ? 'white' : 'var(--text-mid)', fontSize: '0.8rem', fontWeight: 600, cursor: 'pointer' }}>
            All
          </button>
          {categories.map(c => (
      ...
      {cartMsg && (
        <div style={{ position: 'fixed', bottom: '2rem', right: '2rem', background: 'var(--green-deep)', color: 'white', padding: '0.75rem 1.5rem', borderRadius: 10, fontWeight: 600, fontSize: '0.9rem', zIndex: 1000, boxShadow: '0 4px 20px rgba(0,0,0,0.15)' }}>
          {cartMsg}
        </div>
      )}
                ...
                <div style={{ padding: '1.25rem' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '0.75rem' }}>
                    <div>
                      {p.sale_price ? (
                        <>
                          <span style={{ fontFamily: 'var(--font-display)', fontSize: '1.1rem', color: 'var(--green-deep)', fontWeight: 700 }}>NPR {p.sale_price.toLocaleString()}</span>
                          <span style={{ fontSize: '0.8rem', color: 'var(--text-light)', textDecoration: 'line-through', marginLeft: '0.5rem' }}>NPR {p.price?.toLocaleString()}</span>
                        </>
                      ) : (
          ...
          <div style={{ position: 'absolute', right: 0, top: 0, bottom: 0, width: '100%', maxWidth: 420, background: 'var(--white)', boxShadow: '-4px 0 24px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column' }}>
            ...
            {cart.length > 0 && (
              <div style={{ padding: '1.5rem', borderTop: '1px solid var(--earth-cream)' }}>
                ...
                <button onClick={checkout} style={{ width: '100%', padding: '0.9rem', background: 'var(--green-deep)', color: 'white', border: 'none', borderRadius: 10, fontWeight: 700, fontSize: '0.95rem', cursor: 'pointer' }}>
                  Proceed to Checkout →
                </button>
              </div>
            )}
