...
export default function ServiceDetailPage() {
  ...
  return (
    <div className="page-wrapper">
      ...
      >
        <div style={{ position: 'absolute', width: 220, height: 220, borderRadius: '50%', background: 'rgba(0,123,168,0.12)', filter: 'blur(32px)', top: -40, right: '5%', pointerEvents: 'none' }} />
        <div style={{ position: 'absolute', width: 180, height: 180, borderRadius: '50%', background: 'rgba(29,158,117,0.1)', filter: 'blur(32px)', bottom: -20, left: '8%', pointerEvents: 'none' }} />
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 640, margin: '0 auto' }}>
          <button
            onClick={() => navigate('/services')}
            style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#007ba8', fontWeight: 600, fontSize: '0.9rem', marginBottom: '1.25rem', display: 'inline-flex', alignItems: 'center', gap: '0.35rem' }}
          >
            ← All Services
          </button>
          ...
          {service.specialties?.length > 0 && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem', justifyContent: 'center', marginTop: '1rem' }}>
              {service.specialties.map(tag => (
                <span key={tag} style={{ fontSize: '0.75rem', fontWeight: 600, padding: '0.25rem 0.7rem', borderRadius: '999px', background: 'rgba(29,158,117,0.1)', border: '1px solid rgba(29,158,117,0.15)', color: '#1d9e75' }}>
                  {tag}
                </span>
              ))}
            </div>
      ...
      <div className="section" style={{ background: 'var(--white)' }}>
        <div style={{ maxWidth: 900, margin: '0 auto', display: 'grid', gap: '2.5rem' }}>
          ...
          {/* Quick facts — only show if at least one exists */}
          {(service.duration || service.format || service.frequency) && (
            <div
              style={{
                display: 'grid',
            ...
            >
              {service.duration  && <QuickFact label="Duration"  value={service.duration} />}
              {service.format    && <QuickFact label="Format"    value={service.format} />}
              {service.frequency && <QuickFact label="Frequency" value={service.frequency} />}
            </div>
          )}
              ...
              <div style={{ display: 'grid', gap: '1rem' }}>
                {service.process.map((step, i) => (
                  <div key={i} style={{ display: 'flex', gap: '1rem', alignItems: 'flex-start' }}>
                    <div style={{ flexShrink: 0, width: 32, height: 32, borderRadius: '50%', background: 'linear-gradient(135deg, #1d9e75, #007ba8)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.85rem', fontWeight: 700 }}>
                      {i + 1}
                    </div>
                    <div>
            ...
            <section>
              ...
              <div style={{ display: 'grid', gap: '0.75rem' }}>
                {service.faqs.map((faq, i) => (
                  <details key={i} style={{ background: GLASS.bg, border: GLASS.border, borderRadius: '14px', padding: '1rem 1.25rem' }}>
                    <summary style={{ cursor: 'pointer', fontWeight: 600, color: '#213a34' }}>{faq.q}</summary>
          ...
          <div style={{ textAlign: 'center', padding: '1.5rem 0' }}>
            <button
              ...
              style={{ background: 'linear-gradient(135deg, #007ba8 0%, #00bfff 100%)', boxShadow: '0 6px 18px rgba(0,150,210,0.3)', border: 'none', padding: '0.9rem 2.2rem', fontSize: '1rem' }}
