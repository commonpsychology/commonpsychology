...
// Falls back to FALLBACK data if API is unreachable OR returns invalid video IDs.
...
function sanitizeVideos(videos) {
  ...
  const valid = videos.filter(v => v?.youtube_id && isValidYouTubeId(v.youtube_id))
  return valid.length > 0 ? valid : null
}

// ── FALLBACK (shown when API is unreachable or returns bad video IDs) ─────────
// ✏️  TO CHANGE VIDEOS: edit the `videos` array below.
//     youtube_id = the code after ?v= in any YouTube URL
//     e.g. https://www.youtube.com/watch?v=GwEB9lG-sKo  →  youtube_id: 'GwEB9lG-sKo'
const FALLBACK = {
  videos: [
    {
      id: 'f1',
      youtube_id: 'DxIDKZHW3-E',
      title: 'We all Have Mental Health',
      description: 'Common Aspect of a human existence is the mental health at individual level',
      duration: '5:39',
      views: '2.8M views',
    },
    {
      id: 'f2',
      youtube_id: 'zeu9X88g8DE',
      title: 'Well Being of Children',
      description: 'Exploring the mental well being of children',
      duration: '7:43',
      views: '471K views',
    },
    {
      id: 'f3',
      youtube_id: 'rkZl2gsLUp4',
      title: 'How to Manage your Mental Health',
      description: 'Tips and tricks to manage your mental health, and how to seek help when needed, because your mental health matters.',
      duration: '15:07',
      views: '2M views',
    },
  ],
...
// ── YOUTUBE THUMBNAIL with quality fallback chain ─────────────
// Tries: hqdefault → mqdefault → sddefault → error placeholder
function YTThumbnail({ youtubeId, title, isCenter, isHov }) {
  const qualities = ['hqdefault', 'mqdefault', 'sddefault']
  const [qualityIndex, setQualityIndex] = useState(0)
  const [errored, setErrored] = useState(false)

  // Reset when youtubeId changes (e.g. sliding to new card)
  useEffect(() => {
    setQualityIndex(0)
    setErrored(false)
  }, [youtubeId])
  ...
  if (errored) {
    return (
      <div style={{
        ...
        background: 'linear-gradient(135deg,#0a1628,#1a3a4a)',
        display: 'flex', flexDirection: 'column',
        alignItems: 'center', justifyContent: 'center', gap: 8,
      }}>
        <span style={{ fontSize: '2rem', opacity: 0.35 }}>🎬</span>
        <span style={{ fontSize: '0.65rem', color: 'rgba(255,255,255,0.25)', textAlign: 'center', padding: '0 1rem' }}>
          Preview unavailable
        </span>
      </div>
    )
  ...
  return (
    <img
      key={`${youtubeId}-${qualities[qualityIndex]}`}
      src={`https://img.youtube.com/vi/${youtubeId}/${qualities[qualityIndex]}.jpg`}
      alt={title}
      onError={handleError}
      style={{
...
function VideoSlider({ videos, loading }) {
  ...
  // ── helper: open YouTube safely (works in all browsers) ──
  ...
  return (
    <div style={{ background: 'linear-gradient(160deg,#0a1628 0%,#0f2744 60%,#0a1e1a 100%)', padding: '4rem 2rem', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          ...
          {items.map((video, i) => {
            ...
            return (
              <div
                ...
                onClick={() => {
                  if (!isCenter) {
                    // first click brings card to center
                    setCurrent(i)
                  } else {
                    // click on already-centered card opens YouTube
              ...
              >
                <div style={{ position: 'relative', aspectRatio: '16/9', background: '#0a1628', overflow: 'hidden' }}>
                  ...
                  <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <div style={{
                      width: isCenter ? 52 : 38, height: isCenter ? 52 : 38,
                      borderRadius: '50%',
                      background: isCenter ? 'rgba(255,255,255,0.95)' : 'rgba(255,255,255,0.45)',
                  ...
                  <div style={{ position: 'absolute', bottom: 8, right: 8, background: 'rgba(0,0,0,0.78)', borderRadius: 4, padding: '2px 6px', fontSize: '0.68rem', color: 'white', fontWeight: 600 }}>
                    {video.duration}
                  </div>

                  {isCenter && (
                    <div style={{ position: 'absolute', top: 8, left: 8, background: '#FF0000', borderRadius: 4, padding: '2px 7px', fontSize: '0.62rem', color: 'white', fontWeight: 800, letterSpacing: '0.03em' }}>
                      ▶ YouTube
                    </div>
                  )}

                  {/* "Click to select" hint on non-center hover */}
                  {!isCenter && isHov && (
                    <div style={{ position: 'absolute', bottom: 8, left: 8, background: 'rgba(56,189,248,0.85)', borderRadius: 4, padding: '2px 7px', fontSize: '0.62rem', color: 'white', fontWeight: 700 }}>
                      Click to select
                    </div>
                  )}
                </div>
        ...
        {/* YouTube channel link — uses <a> tag, never blocked by browsers */}
        <div style={{ textAlign: 'center', marginTop: '1.75rem' }}>
          <a
            ...
            style={{
              ...
              background: '#FF0000', border: 'none', borderRadius: 100,
...
export default function PsychologicalViewPage() {
  ...
  useEffect(() => {
    ...
    fetchPsychData()
      .then(d => {
        // Validate video YouTube IDs — fall back to FALLBACK.videos if any are bad
        const validVideos = sanitizeVideos(d?.videos)
        const videosOk = !!validVideos

        setData({
          ...d,
          videos:   videosOk                   ? validVideos      : FALLBACK.videos,
          analyses: d?.analyses?.length         ? d.analyses       : FALLBACK.analyses,
          concepts: d?.concepts?.length         ? d.concepts       : FALLBACK.concepts,
        })

        // Show warning banner only if videos had to fall back
        setApiError(!videosOk)
      })
      .catch(() => {
        setData(FALLBACK)
        setApiError(true)
