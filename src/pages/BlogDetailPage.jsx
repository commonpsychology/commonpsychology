...
const mdComponents = {
  h2: ({ children }) => (
    <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.15rem, 2.5vw, 1.4rem)', color: C.textDark, margin: '2rem 0 0.75rem', lineHeight: 1.3, borderBottom: `2px solid ${C.skyFaint}`, paddingBottom: '0.5rem' }}>
      {children}
    </h2>
  ),
  h3: ({ children }) => (
    <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1rem, 2vw, 1.1rem)', color: C.textMid, margin: '1.5rem 0 0.5rem', lineHeight: 1.3 }}>
      {children}
    </h3>
  ),
  p: ({ children }) => (
    <p style={{ fontFamily: 'var(--font-body)', fontSize: 'clamp(0.9rem, 2vw, 0.97rem)', color: C.textMid, lineHeight: 1.85, margin: '0 0 1rem' }}>
      {children}
    </p>
  ),
  ...
  li: ({ children }) => (
    <li style={{ fontFamily: 'var(--font-body)', fontSize: 'clamp(0.88rem, 2vw, 0.95rem)', color: C.textMid, lineHeight: 1.8, marginBottom: '0.35rem' }}>
      {children}
    </li>
  ),
  ...
  code: ({ children }) => (
    <code style={{ background: C.skyFaint, color: C.skyDeep, padding: '2px 6px', borderRadius: 4, fontSize: '0.88rem', fontFamily: 'monospace', wordBreak: 'break-word' }}>
      {children}
    </code>
  ),
}
...
export default function BlogDetailPage() {
  ...
  if (error || !post) return (
    <div style={{ minHeight: '70vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: '1rem', background: C.skyGhost, padding: '1rem' }}>
      <div style={{ fontSize: '3.5rem' }}>😕</div>
      <p style={{ fontFamily: 'var(--font-body)', color: C.textLight, fontSize: '1.05rem', textAlign: 'center' }}>Article not found.</p>
    ...
    <>
      <style>{`
        ...
        @media (max-width: 1024px) {
          .blog-hero { padding: 5rem 2.5rem 2.5rem; }
          .blog-main-grid {
        ...
        @media (max-width: 768px) {
          .blog-hero { padding: 4.5rem 1.25rem 2rem; }
          .blog-main-grid {
        ...
        @media (max-width: 480px) {
          .blog-hero { padding: 4rem 1rem 1.75rem; }
      ...
      <div style={{ background: C.skyGhost, minHeight: '100vh' }}>
        ...
        <div className="blog-hero">
          <div className="blog-hero-inner">
            ...
            <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.4rem, 3.5vw, 2.4rem)', color: 'white', lineHeight: 1.25, maxWidth: 800, marginBottom: '1.25rem' }}>
              {post.title}
            </h1>

            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap' }}>
              <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'rgba(255,255,255,0.25)', border: '2px solid rgba(255,255,255,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.85rem', color: 'white', fontWeight: 700, flexShrink: 0 }}>
              ...
              <div style={{ marginLeft: 'auto', fontFamily: 'var(--font-body)', fontSize: '0.75rem', color: 'rgba(255,255,255,0.65)', whiteSpace: 'nowrap' }}>
        ...
        <div className="blog-main-grid">
          ...
          <div>
            ...
            <div style={{ background: `linear-gradient(135deg, ${C.skyFainter}, ${C.mint})`, border: `1px solid ${C.borderFaint}`, borderLeft: `4px solid ${C.skyBright}`, borderRadius: '0 14px 14px 0', padding: '1.25rem 1.5rem', marginBottom: '2rem' }}>
              <p style={{ fontFamily: 'var(--font-body)', fontSize: 'clamp(0.9rem, 2vw, 1rem)', color: C.textMid, lineHeight: 1.75, margin: 0, fontStyle: 'italic' }}>
                {post.excerpt}
              </p>
            </div>
            ...
            {post.content ? (
              ...
              <div style={{ padding: '2.5rem', textAlign: 'center', background: C.white, borderRadius: 14, border: `1px dashed ${C.borderFaint}` }}>
                ...
                <p style={{ fontFamily: 'var(--font-body)', color: C.textLight, fontSize: '0.9rem' }}>Full article content coming soon.</p>
              </div>
            )}
          ...
          <div className="blog-sidebar">
            ...
            {/* Article image */}
            <div style={{ borderRadius: 16, overflow: 'hidden', boxShadow: `0 8px 32px rgba(0,191,255,0.15)`, background: post.gradient || heroGrad, minHeight: 200, position: 'relative' }}>
              {post.image_url && (
                <img
                  src={post.image_url}
                  alt={post.title}
                  onLoad={() => setImgLoaded(true)}
                  style={{ width: '100%', height: '100%', minHeight: 200, objectFit: 'cover', display: 'block', opacity: imgLoaded ? 1 : 0, transition: 'opacity 0.4s ease' }}
                />
              )}
              {!imgLoaded && !post.image_url && (
                <div style={{ minHeight: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '3rem' }}>📝</div>
              )}
            </div>
            ...
            <div style={{ background: C.white, borderRadius: 14, border: `1px solid ${C.borderFaint}`, padding: '1.25rem', boxShadow: `0 2px 12px rgba(0,191,255,0.06)` }}>
              ...
              {[
                ...
                <div key={label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.5rem 0', borderBottom: `1px solid ${C.borderFaint}`, gap: '0.5rem' }}>
                  <span style={{ fontFamily: 'var(--font-body)', fontSize: '0.78rem', color: C.textLight, flexShrink: 0 }}>{label}</span>
                  <span style={{ fontFamily: 'var(--font-body)', fontSize: '0.78rem', fontWeight: 700, color: C.textDark, textAlign: 'right' }}>{value}</span>
                </div>
              ))}
            </div>
            ...
            <div style={{ background: `linear-gradient(135deg, ${C.skyFainter}, ${C.mint})`, borderRadius: 14, border: `1px solid ${C.borderFaint}`, padding: '1.25rem' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                ...
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontFamily: 'var(--font-body)', fontSize: '0.88rem', fontWeight: 700, color: C.textDark, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{post.author}</div>
