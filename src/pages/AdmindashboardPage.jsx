...
const CSS = `
  ...
  .adm-topbar {
    background: linear-gradient(135deg,#1a3a4a 0%,#2e6080 100%);
  ...
  .adm-sidebar-btn.active::before {
    content: ''; position: absolute; left: 0; top: 15%; bottom: 15%;
    width: 3px; background: #1a7a4a; border-radius: 0 2px 2px 0;
  }
  ...
  .adm-tabbar-btn {
    ...
    color: #7a9aaa; cursor: pointer; white-space: nowrap;
  ...
  .adm-pager-btn:disabled { background: #f0f4f8; cursor: not-allowed; }
  /* Filters */
  ...
  @media (max-width: 680px) {
    ...
    .adm-topbar   { padding: 0.6rem 1rem; }
  }
  /* ── Small mobile 420px ── */
...
export default function AdminDashboard() {
  ...
  const fetchPays = useCallback(async () => {
    setB('pays',true)
    try { const d = await adminApi.payments({ page:pPage, limit:LIMIT }); setPays(d.payments||[]); setPTotal(d.pagination?.total||0) }
    ...
    <div className="adm-layout">
      ...
      <div className="adm-topbar">
        ...
        <div style={{ display:'flex', alignItems:'center', gap:'0.65rem' }}>
          <div style={{ textAlign:'right', display:'none' }} className="adm-user-info">
            <div style={{ fontSize:'0.83rem', color:'white', fontWeight:600 }}>{user?.fullName}</div>
            <div style={{ fontSize:'0.68rem', color:'rgba(255,255,255,0.55)' }}>{user?.email}</div>
          </div>
          <div style={{ width:32, height:32, borderRadius:'50%', background:'rgba(255,255,255,0.18)', display:'flex', alignItems:'center', justifyContent:'center', color:'white', fontWeight:800, fontSize:'0.82rem', flexShrink:0 }}>
      ...
      <div className="adm-body">
        ...
        <div className="adm-sidebar">
          ...
          <button className="adm-sidebar-btn" onClick={() => navigate('/')}>
            <span>🏠</span> Main Site
          </button>
        </div>
        ...
        <div className="adm-content">
                ...
                {busy.dash ? <p style={{ color:'#7a9aaa', fontSize:'0.82rem' }}>Loading…</p>
                  ...
                  : recent.map((p,i) => (
                    <div key={i} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'0.65rem 0', borderBottom:i<recent.length-1?'1px solid #e2e8f0':'none' }}>
                      <div>
                        <div style={{ fontSize:'0.85rem', color:'#1a3a4a', fontWeight:600, fontFamily:'var(--font-body)' }}>NPR {Number(p.amount).toLocaleString()}</div>
                        <div style={{ fontSize:'0.72rem', color:'#7a9aaa', fontFamily:'var(--font-body)' }}>{p.method} · {fmt(p.created_at)}</div>
                      </div>
                      <Badge s={p.status}/>
                    </div>
                  ))
            ...
            <div>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'1.25rem', flexWrap:'wrap', gap:'0.75rem' }}>
                ...
                <div className="adm-filters" style={{ margin:0 }}>
                  <input className="adm-search" value={uSearch} onChange={e => setUSearch(e.target.value)} onKeyDown={e => { if(e.key==='Enter'){ setUPage(1); fetchUsers() }}} placeholder="Search name or email…" style={{ width:'clamp(140px,30vw,220px)' }}/>
              ...
              <TableShell loading={busy.users} cols={['User','Email','Role','Status','Joined','Actions']}
                rows={users.map(u => (
                  <tr key={u.id}>
                    ...
                    <td style={{ color:'#7a9aaa', fontSize:'0.78rem', fontFamily:'var(--font-body)' }}>{u.email}</td>
                    <td>
                      <select className="adm-select" value={u.role} onChange={e => doRole(u.id,e.target.value)} style={{ padding:'0.22rem 0.5rem', fontSize:'0.78rem' }}>
                        {['client','therapist','admin','staff'].map(r => <option key={r} value={r}>{r}</option>)}
                      </select>
                    </td>
                    <td><Badge s={u.is_active?'active':'paused'}/></td>
                    <td style={{ color:'#7a9aaa', fontSize:'0.78rem', fontFamily:'var(--font-body)' }}>{fmt(u.created_at)}</td>
                    <td>
                      <button onClick={() => doToggle(u.id)}
                        style={{ padding:'0.28rem 0.7rem', border:'1px solid #e2e8f0', borderRadius:6, fontSize:'0.74rem', cursor:'pointer', background:'#fff', color:u.is_active?'#c0392b':'#1a7a4a', fontWeight:700, fontFamily:'var(--font-body)' }}>
          ...
          {tab==='appointments' && (
            <div>
              ...
              <TableShell loading={busy.appts} cols={['Client','Therapist','Date & Time','Type','Status','Update']}
                rows={appts.map(a => (
                  <tr key={a.id}>
                    <td style={{ fontFamily:'var(--font-body)' }}><strong style={{ color:'#1a3a4a' }}>{a.clients?.full_name||'—'}</strong></td>
                    <td style={{ fontFamily:'var(--font-body)' }}>{a.therapists?.profiles?.full_name||'—'}</td>
                    <td style={{ fontSize:'0.78rem', color:'#7a9aaa', fontFamily:'var(--font-body)' }}>{fmtT(a.scheduled_at)}</td>
                    <td style={{ fontFamily:'var(--font-body)' }}><span style={{ fontSize:'0.75rem', background:'#f0f4f8', padding:'0.18rem 0.5rem', borderRadius:5 }}>{a.type}</span></td>
                    <td><Badge s={a.status}/></td>
                    <td>
                      <select className="adm-select" value={a.status} onChange={e => doApptStatus(a.id,e.target.value)} style={{ padding:'0.25rem 0.45rem', fontSize:'0.77rem' }}>
                        {['pending','confirmed','completed','cancelled','no_show'].map(s => <option key={s} value={s}>{s}</option>)}
                      </select>
                    </td>
                  </tr>
          ...
          {tab==='orders' && (
            <div>
              ...
              <TableShell loading={busy.orders} cols={['Order #','Client','Amount','Status','Date','Update']}
                rows={orders.map(o => (
                  <tr key={o.id}>
                    <td style={{ fontWeight:700, color:'#1a7a4a', fontSize:'0.8rem', fontFamily:'var(--font-body)' }}>{o.order_number||'—'}</td>
                    <td style={{ fontFamily:'var(--font-body)' }}>{o.profiles?.full_name||'—'}</td>
                    <td style={{ fontFamily:'var(--font-body)' }}><strong>NPR {Number(o.total_amount||0).toLocaleString()}</strong></td>
                    <td><Badge s={o.status}/></td>
                    <td style={{ fontSize:'0.78rem', color:'#7a9aaa', fontFamily:'var(--font-body)' }}>{fmt(o.created_at)}</td>
                    <td>
                      <select className="adm-select" value={o.status} onChange={e => doOrderStatus(o.id,e.target.value)} style={{ padding:'0.25rem 0.45rem', fontSize:'0.77rem' }}>
                        {['pending','confirmed','processing','shipped','delivered','cancelled','refunded'].map(s => <option key={s} value={s}>{s}</option>)}
                      </select>
                    </td>
                  </tr>
          ...
          {tab==='payments' && (
            <div>
              <h1 style={{ fontFamily:'var(--font-display)', fontSize:'clamp(1.2rem,3vw,1.4rem)', color:'#1a3a4a', margin:'0 0 1.25rem' }}>
                Payments <span style={{ fontSize:'0.9rem', color:'#7a9aaa', fontWeight:400 }}>({pTotal})</span>
              </h1>
              <TableShell loading={busy.pays} cols={['Client','Amount','Method','Status','Transaction ID','Date']}
                rows={pays.map(p => (
                  <tr key={p.id}>
                    <td style={{ fontFamily:'var(--font-body)' }}>{p.profiles?.full_name||'—'}</td>
                    <td style={{ fontFamily:'var(--font-body)' }}><strong>NPR {Number(p.amount||0).toLocaleString()}</strong></td>
                    <td style={{ textTransform:'capitalize', fontSize:'0.8rem', fontFamily:'var(--font-body)' }}>{p.method||'—'}</td>
                    <td><Badge s={p.status}/></td>
                    <td style={{ fontSize:'0.78rem', color:'#7a9aaa', fontFamily:'var(--font-body)' }}>{p.transaction_id||'—'}</td>
                    <td style={{ fontSize:'0.78rem', color:'#7a9aaa', fontFamily:'var(--font-body)' }}>{fmt(p.created_at)}</td>
                  </tr>
                ))}
              />
